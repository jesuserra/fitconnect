...
export default function FlagDropdown({ options, style, error, onChange, defaultValue = 'Todos' }: FlagDropdownProps) {
          ...
          {options.map((option, index) => (
            <a href="#" key={index} onClick={() => handleOptionClick(option)}>
              <div key={index} className='items-center gap-2 flex px-4 py-2 hover:bg-gray-100 dark:hover:bg-[#161d2a] pointer Dialog Dialog text-gray-400'>
                  <img className="w-6 h-4" src={`https://flagcdn.com/${option}.svg`} alt="User Country Flag"/>
                  <span className='text-base'>{countryMap[option as keyof typeof countryMap]}</span>
              </div>
            </a>
          ))}
        </div>
